...
python -c "import os,ctypes,glob; d=os.path.abspath('%BACKEND_DIR%'); os.add_dll_directory(d); os.environ['PATH']=d+os.pathsep+os.environ.get('PATH',''); [exec('try: ctypes.CDLL(p,winmode=0)\nexcept: pass', {'ctypes':ctypes, 'p':p}) for p in sorted(glob.glob(os.path.join(d,'ggml*.dll')))]; q=ctypes.CDLL(os.path.join(d,'qwen.dll'),winmode=0); q.qt_version.restype=ctypes.c_char_p; q.qt_version.argtypes=[]; print('        qwen.dll loaded, version:', q.qt_version().decode())"
if errorlevel 1 (
